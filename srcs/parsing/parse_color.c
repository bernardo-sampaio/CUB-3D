...
/*   Updated: 2026/05/17 18:33:35 by ealbino          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
    ...
    while (color && color[i])
    {
        if (color[i] == ',')
        {
            if (check_comma_pos(color, i) == false)
                return (error_msg("After/Before comma [,] should have a number"), false);
            count++;
        }
        i++;
    }
    ...
    while (head)
    {
        ...
        if (pick_c != NULL)
        {
            ...
            if (all_digit(mat) == false)
            {
                free(pick_c);
                free_mat(mat);   
                return (error_msg("Color should be just number [0 - 9]"), false);
            }
            if (validate_range(mat) == false)
            {
                free(pick_c);
                free_mat(mat);   
                return (error_msg("Color's range should be between [0 - 255]"), false);
            }
